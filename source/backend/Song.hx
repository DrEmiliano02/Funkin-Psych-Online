...
class Song
{
	...
	// Optimizado: Mapeo de nombres antiguos → nuevos
	...
	private static function onLoadJson(songJson:Dynamic)
	{
		...
		if (songJson.events == null && songJson.format == 'psych_legacy')
		{
			...
			{
				sec.sectionNotes = sec.sectionNotes.filter(function(note:Array<Dynamic>)
				{
					if (note[1] < 0) {
	...
	public static function loadRawSong(jsonInput:String, ?folder:String):String
	{
		...
		// Optimizado: limpiar en una sola operación con regex
