	...
	override public function update(elapsed:Float)
	{
		...
		else if (!paused && updateTime)
		{
			...
			if(ClientPrefs.data.timeBarType != 'Song Name')
				var curSec = Std.int(secondsTotal / playbackRate);
if (curSec != _lastShownSec && ClientPrefs.data.timeBarType != 'Song Name') {
    timeTxt.text = FlxStringUtil.formatTime(curSec, false);
    _lastShownSec = curSec;
}}
...
if (!GameClient.isConnected()
    ...
    && isPlayNoteNear)
{
    var desync = Math.abs(FlxG.sound.music.time - (Conductor.songPosition - Conductor.offset));
    if (desync >= 350) {
        setSongTime(FlxG.sound.music.time);
        lastLagPos = Conductor.songPosition + 8000; // cooldown más largo
        #if debug
        Alert.alert("Desync corregido (snap to audio)");
        #end
    }
}
